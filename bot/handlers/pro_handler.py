...
Запиши 🎙 для тех, кто выбрал 'Платить меньше'.
...
> _"Поэтому они готовы отдавать эти номера закрытым клубам, типа нашего, со скидкой 50-70%, лишь бы заполнить."_

> _"Но они запрещают показывать эти цены публично на Букинге или Островке, чтобы не рушить рынок. А нам внутри клуба — можно. Вот и весь секрет. Ты просто получаешь доступ к оптовым ценам, которые скрыты от обычных туристов."_
...
Запиши 🎙 для тех, кто выбрал 'Жить в 5★ по цене 3★'.
Бот отправит его в ветке "Путешествия".

📝 **Сценарий:**

"Слушай, я раньше всегда думал: ну нафига переплачивать за отель, я ж там только сплю? 

А потом понял, что я так говорил, просто потому что денег жалел.
 
А тут фишка в чем... 

Ты можешь за те же самые деньги — вот реально за те же, что планировал потратить на обычный отель — взять уровень на голову выше.
 
Ну то есть, вместо обычной гостиницы — взять крутой отель с бассейном на крыше или со своим пляжем.

Бюджет тот же, а впечатления вообще другие. Вот это для меня самое ценное здесь."
...
# Обработчик ветки "Уволиться из найма" (голосовое)
@router.callback_query(F.data == "upload_quit_job_voice")
async def upload_quit_job_voice_prompt(callback: CallbackQuery, state: FSMContext):
    """Запрос на загрузку голосового для ветки Уволиться из найма"""
    
    prompt_text = """🚀 **Ветка: Уволиться из найма**
    
Запиши 🎙 для тех, кто выбрал 'Уволиться из найма'.
Бот отправит его в ветке "Бизнес".

📝 **Сценарий:**

"Слушай, я тебя прекрасно понимаю. Уходить в никуда — это огромный стресс, и я точно не буду тебе говорить «бросай всё и беги к нам». Это глупо.
...
⏳ **Жду голосовое...**"""
    ...
    try:
        ...
        if "message is not modified" in str(e):
            # Если сообщение не изменилось, просто отправляем ответ на callback
            await callback.answer("Запись 'Уволиться из найма'", show_alert=False)
        else:
            # Если другая ошибка BadRequest, пробрасываем дальше
            raise
...
async def save_quit_job_voice(message: Message, state: FSMContext, session: AsyncSession):
    """Сохранение голосового для ветки Уволиться из найма"""
    ...
    await message.answer_voice(voice=voice_id)
    await message.answer(
        "✅ **Готово! Голосовое для ветки 'Уволиться из найма' сохранено.**",
        reply_markup=get_back_to_personalization(),
        parse_mode="Markdown"
    )
...
async def wrong_quit_job_voice_type(message: Message):
    """Обработка неверного типа контента"""
...
Запиши 🎙 для Финала.
Оно придет перед кнопкой входа в Приложение.

📝 **Сценарий:**

Короче, система полностью готова. 

Я уже внутри. 

Переходи в Приложение, там лежат уроки и инструменты. 

Если что — пиши мне в личку, контакты там же. До встречи в команде!"
...
"Слушай, тут всё на самом деле просто. 

Отели — это скоропортящийся продукт. 

Если номер сегодня ночью пустой, отель потерял деньги навсегда.

Поэтому они готовы отдавать эти номера закрытым клубам, типа нашего, со скидкой 50-70%, лишь бы заполнить.

Но они запрещают показывать эти цены публично на Букинге или Островке, чтобы не рушить рынок. 

А нам внутри клуба — можно. Вот и весь секрет. 

Ты просто получаешь доступ к оптовым ценам, которые скрыты от обычных туристов."
...
Запиши 🎙 для тех, кто выбрал 'Путешествовать чаще'.
Бот отправит его в ветке "Путешествия".

📝 **Сценарий:**

> _"Слушай, мы все хотим путешествовать чаще, но вечно откладываем это «на потом».
>
> Всегда находятся дела поважнее, куда потратить деньги."_
>
> _"А здесь система построена так, что ты по чуть-чуть откладываешь себе на счет — как подписка.
>
> Только эти деньги не сгорают, они копятся.
>
> И в какой-то момент ты заходишь в приложение и видишь: о, у меня уже на целый отпуск собралось!
>
> И ты просто берешь и летишь.
>
> Не потому что надо напрягаться искать деньги, а потому что они уже там.
>
> Это реально самый простой способ начать ездить куда-то 3-4 раза в год."_
...
Запиши 🎙 для финала ветки "Пассивный доход".
Бот отправит его перед кнопкой входа в Приложение (Бизнес Хаб).

📝 **Сценарий:**

"Вообщем, я не хочу тебе тут расписывать поэмы.

Я подготовил для тебя доступ в наше приложение. 

Там есть всё: калькулятор, моя система, конкретные цифры. 

Зайди, потыкай кнопки, посчитай сам свою выгоду.

Если увидишь в этом перспективу — там внутри есть мои контакты. 

Напиши, обсудим стратегию. 
...
Запиши ГС для тех, кто выбрал 'Путешествовать бесплатно'.
Бот отправит его в ветке "Бизнес".

📝 **Сценарий:**

"Слушай, самая гениальная вещь здесь — это то, что ты можешь вообще убрать из своего бюджета расходы на отпуск.

Мы все тратим на это кучу денег каждый год. 

А тут компания говорит: «Пользуйся сам, расскажи друзьям, и если они тоже начнут экономить — мы будем оплачивать твои путешествия за тебя».

Ты просто вдумайся. 

Ты не продаешь им пылесосы, ты даешь им способ экономить. 

И за это компания начинает пополнять твой счет на путешествия каждый месяц. Бесплатно. 

Я когда понял эту математику, я вообще перестал переживать о ценах на билеты."
...
Запиши 🎙 для финала ветки 🌍 Путешествовать бесплатно.
Бот отправит его перед кнопкой входа в Приложение.

📝 **Сценарий:**

"Смотри, чтобы у тебя не было ощущения, что надо за кем-то бегать. 

Это не работает.

Весь секрет в том, что мы просто показываем людям, как они могут сэкономить свои же деньги. 

Мы не просим у них денег, мы даем им выгоду. 

А когда ты даешь человеку выгоду — тебе не надо его «искать» или уговаривать, он сам хочет.

В приложении я показал именно этот подход: как сделать так, чтобы твое окружение само интересовалось, откуда у тебя такие цены. 

Зайди, посмотри эту стратегию. Это про достоинство, а не про впаривание."
...
Запиши 🎙 для финала ветки "Уволиться из найма".
Бот отправит его перед кнопкой входа в Приложение.

📝 **Сценарий:**

"Смотри, я не хочу грузить тебя теорией. Лучше один раз увидеть._

Я открыл тебе доступ в наше приложение. 

Там ты увидишь, как работает наша система изнутри. 

Не на словах, а на деле — какие инструменты мы используем и почему этот механизм работает так надежно.

Зайди, спокойно всё изучи, «примерь» на себя. 

Если почувствуешь, что тебе эта модель откликается — напиши мне, я подскажу, с чего начать."_
